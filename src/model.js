...
export function restore(raw) {
  ...
  const template = seed();
  if (value.version !== 1 || Object.keys(template).some(k => !(k in value)) || Object.keys(template).filter(k => Array.isArray(template[k])).some(k => !Array.isArray(value[k])) || !value.settings || typeof value.settings !== 'object') throw new Error('Saved data is incompatible. Export or reset the workspace in Settings.');
...
export function transition(state, action) {
  ...
  switch (action.type) {
    ...
    case 'SIGNUP': {
      if (!['company', 'contractor', 'worker'].includes(d.role)) fail('Choose a role.');
    ...
    case 'ATTENDANCE': {
      ...
      if (!['Present', 'Half day', 'Absent'].includes(d.status)) fail('Choose an attendance status.');
    ...
    case 'EXPENSE': requireRole('contractor'); active(d.projectId); s.expenses.unshift({ id: uid, contractorId: actor.id, projectId: d.projectId, category: text(d.category, 'Category'), amount: number(d.amount, 'Expense'), date: today(), note: text(d.note, 'Description') }); break;
    case 'RENTAL': {
    ...
    case 'SETTINGS': requireRole('company', 'contractor', 'worker'); s.settings[actor.id] = { notifications: d.notifications === 'On', compact: d.compact === 'Compact' }; break;
    case 'READ': requireRole('company', 'contractor', 'worker'); s.notifications.filter(n => n.userId === actor.id && (!d.notificationId || n.id === d.notificationId)).forEach(n => { n.read = true; }); break;
